...
data Tree :: * -> * where
  ...
  GOneCommand :: GPosCommand -> Tree GCommands_
  ...
  GCompoundCommand :: GConjunct -> GListPosCommand -> Tree GPosCommand_
  GDoTil :: GAction -> GTime -> Tree GPosCommand_
  GFinish :: Tree GPosCommand_
...
instance Eq (Tree a) where
  i == j = case (i,j) of
    ...
    (GCompoundCommand x1 x2,GCompoundCommand y1 y2) -> and [ x1 == y1 , x2 == y2 ]
...
instance Gf GPosCommand where
  gf (GCompoundCommand x1 x2) = mkApp (mkCId "CompoundCommand") [gf x1, gf x2]
  gf (GDoTil x1 x2) = mkApp (mkCId "DoTil") [gf x1, gf x2]
  gf GFinish = mkApp (mkCId "Finish") []
  gf (GSimpleCom x1) = mkApp (mkCId "SimpleCom") [gf x1]

  fg t =
    case unApp t of
      Just (i,[x1,x2]) | i == mkCId "CompoundCommand" -> GCompoundCommand (fg x1) (fg x2)
      Just (i,[x1,x2]) | i == mkCId "DoTil" -> GDoTil (fg x1) (fg x2)
      Just (i,[]) | i == mkCId "Finish" -> GFinish 
...
instance Compos Tree where
  compos r a f t = case t of
    ...
    GCompoundCommand x1 x2 -> r GCompoundCommand `a` f x1 `a` f x2
