...
domain of commanding controllable robots, specifically, autonomous vehicles.
For the actions a vehicle takes, mostly the motion and path decisions, must be
...
the system requires. For her commands may leave out detail (``go into the other lane''
with multiple lanes on either side), say something wrong (``go into the other
lane'' on a single lane road), or give a command the controller should recognize
as bad (``drive directly into the car ahead''). Additionally, the controller may need to
...
the possibility of an adversarial attacks, which we'll detail more below.
...
F (man_with_dog /\ G Finish)
F (man /\ G Finish) /\ G with_dog
...
% Warricks personal (confessed) confusions
